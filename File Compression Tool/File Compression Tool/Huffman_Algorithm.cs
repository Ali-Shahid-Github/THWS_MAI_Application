...
namespace File_Compression_Tool
{
    class Huffman_Algorithm
    {
        ...
        private static Stopwatch watch = new Stopwatch();   //this is used to find the time for execution
                ...
                foreach (var character in input_file_data)
                {
                    ...
                    if (character_codes.ContainsKey(character))
                    {
                        if (ch.Length > 4)
                        {
                            ...
                            int partition = length / 4;             // 4 is the size were we combining the redundtant bits

                            for (int i = 0; i < partition; i++)
                            {
                                ch = ch.Remove(remove_counter++, 4);
                                ch = ch.Insert(insert_counter++, "\0");
